...
import { aleaPRNG } from '../../random.js';
import { createNoise2D, createNoise3D } from 'https://cdn.jsdelivr.net/npm/simplex-noise@4.0.3/+esm';
...
function fbm_noise(noise, nx, ny, nz) {
    let sum = 0, sumOfAmplitudes = 0;
    for (let octave = 0; octave < amplitudes.length; octave++) {
        let frequency = 1 << octave;
        sum += amplitudes[octave] * noise(nx * frequency, ny * frequency, nz * frequency);
...
function generatePlates(ctx, mesh, r_xyz) {
    let r_plate = new Int32Array(mesh.numRegions);
    r_plate.fill(-1);
    let grng = aleaPRNG(ctx.seed);
    let grandInt = (n) => Math.floor(grng() * n);
    let plate_r = pickRandomRegions(mesh, Math.min(ctx.P, ctx.N), grandInt);
    let queue = Array.from(plate_r);
    for (let r of queue) { r_plate[r] = r; }
    let out_r = [];
    let randInt = (n) => Math.floor(aleaPRNG(ctx.seed)() * n);
...
function findCollisions(ctx, mesh, r_xyz, plate_is_ocean, r_plate, plate_vec) {
    ...
    for (let current_r = 0; current_r < numRegions; current_r++) {
        ...
        if (best_r !== -1) {
            let collided = bestCompression > ctx.collision * deltaTime;
...
function assignRegionElevation(ctx, mesh, { r_xyz, plate_is_ocean, r_plate, plate_vec, /* out */ r_elevation }) {
    const epsilon = 1e-3;
    let { numRegions } = mesh;

    let { mountain_r, coastline_r, ocean_r } = findCollisions(
        ctx, mesh, r_xyz, plate_is_ocean, r_plate, plate_vec);
    ...
    let r_distance_a = assignDistanceField(mesh, mountain_r, ocean_r, ctx.seed);
    let r_distance_b = assignDistanceField(mesh, ocean_r, coastline_r, ctx.seed);
    let r_distance_c = assignDistanceField(mesh, coastline_r, stop_r, ctx.seed);
    ...
    for (let r = 0; r < numRegions; r++) {
        ...
        r_elevation[r] += 0.1 * fbm_noise(ctx.noise, r_xyz[3 * r], r_xyz[3 * r + 1], r_xyz[3 * r + 2]);
    }
}
...
function generateMesh(ctx) {
    let t0 = performance.now();
    let result = makeSphere(ctx.N, ctx.jitter, aleaPRNG(ctx.seed));
    let mesh = result.mesh;
    let map = {};
    let quadGeometry = new QuadGeometry();
    ...
    let localCtx = { ...ctx, mesh, map, quadGeometry, noise: new createNoise3D(aleaPRNG(ctx.seed)) };
    generateMapForType(localCtx);
    return { mesh, map, quadGeometry };
}

// Dispatch to planet-type-specific map generation
function generateMapForType(ctx) {
    switch (ctx.planetType) {
        case 'airless': return generateAirlessMap(ctx);
        case 'barren':
            if (ctx.barrenSubtype === 'hostile') return generateHostileMap(ctx);
            return generateBarrenMap(ctx);
        case 'gasgiant': return generateGasGiantMap(ctx);
        case 'sun': return generateSunMap(ctx);
        default: return generateEarthlikeMap(ctx);
    }
}

// Earth-like: plates, 50% ocean, collision elevation, FBM moisture &gt;0.15, rivers, quad geometry
function generateEarthlikeMap(ctx) {
    const { mesh, map, quadGeometry } = ctx;
    let result = generatePlates(ctx, mesh, map.r_xyz);
    ...
    assignRegionElevation(ctx, mesh, map);
    for (let r = 0; r < mesh.numRegions; r++) {
        let n = 0.5 + 0.5 * fbm_noise(ctx.noise, map.r_xyz[3 * r], map.r_xyz[3 * r + 1], map.r_xyz[3 * r + 2]);
...
function generateCraterElevation(ctx, mesh, r_xyz) {
    const numRegions = mesh.numRegions;
    const elevation = new Float32Array(numRegions);
    let rng = aleaPRNG(ctx.seed + 9999);

    for (let r = 0; r < numRegions; r++) {
        elevation[r] = 0.15 * fbm_noise(ctx.noise, r_xyz[3 * r], r_xyz[3 * r + 1], r_xyz[3 * r + 2]);
    }
...
function generateAirlessMap(ctx) {
    const { mesh, map, quadGeometry } = ctx;
    map.plate_r = [0];
    ...
    map.r_elevation = generateCraterElevation(ctx, mesh, map.r_xyz);
...
function generateBarrenMap(ctx) {
    const { mesh, map, quadGeometry } = ctx;
    let result = generatePlates(ctx, mesh, map.r_xyz);
    map.plate_r = result.plate_r;
    map.r_plate = result.r_plate;
    ...
    let rng = aleaPRNG(ctx.seed + 7777);
...
function generateHostileMap(ctx) {
    const { mesh, map, quadGeometry } = ctx;
    let savedP = ctx.P;
    ctx.P = Math.round(ctx.P * 1.5);
    let result = generatePlates(ctx, mesh, map.r_xyz);
    ctx.P = savedP;
    map.plate_r = result.plate_r;
    map.r_plate = result.r_plate;
    ...
    let rng = aleaPRNG(ctx.seed + 8888);
    let out_r = [];
    for (let center_r of map.plate_r) {
        if (rng() > 0.3) continue;
...
function generateGasGiantMap(ctx) {
    const { mesh, map, quadGeometry } = ctx;
    map.plate_r = [0];
    map.r_plate = new Int32Array(mesh.numRegions);
    map.r_plate.fill(0);
...
function generateSunMap(ctx) {
    const { mesh, map, quadGeometry } = ctx;
    map._sunSeed = ctx.seed;
    map.plate_r = [0];
    map.r_plate = new Int32Array(mesh.numRegions);
    map.r_plate.fill(0);
    map.plate_vec = [vec3.fromValues(0, 0, 0)];
...
// Apply temperature/rainfall/water-level offsets to quadGeometry UV data in-place (no mesh rebuild)
export function applyClimate(result, tempOff, rainOff, waterOff) {
    const { mesh, map, quadGeometry } = result;
    quadGeometry.applyClimate(
        mesh.numRegions, mesh.numTriangles,
        map.r_elevation, map.r_moisture,
        map.t_elevation, map.t_moisture,
        tempOff, rainOff, waterOff
    );
}

// Classify a region into a biome label + temperature, type-aware (moved from main.js pickRegion)
export function getRegionInfo(result, region) {
    const { mesh, map } = result;
    const { planetType, barrenSubtype, tempOffset, rainOffset, waterLevel } = result.meta;
    let e = map.r_elevation[region] - waterLevel;
    if (e > 0) {
        e = tempOffset > 0 ? e / (1 + tempOffset * 3) : e * (1 + Math.abs(tempOffset) * 2);
    }
    const m = Math.min(1, Math.max(0, map.r_moisture[region] + rainOffset));
    const plate = map.r_plate[region];
    const isOcean = map.plate_is_ocean.has(plate);
    const rawElevation = map.r_elevation[region];
    ...
    return {
        ...
        biome,
        x: map.r_xyz[3 * region],
        y: map.r_xyz[3 * region + 1],
        z: map.r_xyz[3 * region + 2],
    };
}

// Core entry point: build a complete planet map/mesh from an options bag.
// Returns a self-contained result object digestible by any consumer (incl. renderer).
export function generateMap(options = {}) {
    const ctx = {
        seed: options.seed ?? crypto.randomUUID(),
        N: options.N ?? 20000,
        P: options.P ?? 20,
        collision: options.collision ?? 0.75,
        jitter: options.jitter ?? 0.75,
        rotation: options.rotation ?? -1,
        drawMode: options.drawMode ?? 'quads',
        planetType: options.planetType ?? 'earthlike',
        barrenSubtype: options.barrenSubtype ?? 'barren',
        tempOffset: options.tempOffset ?? 0,
        rainOffset: options.rainOffset ?? 0,
        waterLevel: options.waterLevel ?? 0,
        numCultures: options.numCultures ?? 16,
        numStates: options.numStates ?? 16,
        maxBurgs: options.maxBurgs ?? 10000,
    };

    let { mesh, map, quadGeometry } = generateMesh(ctx);
    ...
    return {
        ...
        meta: {
            ...
            planetType: ctx.planetType,
            barrenSubtype: ctx.barrenSubtype,
            tempOffset: ctx.tempOffset,
            rainOffset: ctx.rainOffset,
