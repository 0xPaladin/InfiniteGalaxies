...
// The raw (elevRaw, moistRaw) noise field is a PURE function of (seed, lon, lat)
// — evaluating it doesn't require the grid to exist. Extracted so field.js
// (IMPLEMENTATION_PLAN.md §11.1) can sample the exact same macro field at
// arbitrary points for region generation, not just at this grid's own jittered
// sample locations. The two noise generators are cached per seed since a
// region samples this thousands of times per generation.
const _noiseCache = new Map();
function noisesFor(seed) {
  let entry = _noiseCache.get(seed);
  if (!entry) {
    entry = {
      elev: makeNoise2D(childSeed(seed, 'elev')),
      moisture: makeNoise2D(childSeed(seed, 'moisture'))
    };
    _noiseCache.set(seed, entry);
  }
  return entry;
}

/** @returns {{elevRaw: number, moistRaw: number}} both in the fbm's native ~[-1,1]-ish range */
export function sampleRawField(seed, lon, lat) {
  const { elev, moisture } = noisesFor(seed);
  const nx = (lon / 180 + 1) * 2;
  const ny = (lat / 90 + 1) * 2;
  return {
    elevRaw: fbm(elev, nx, ny, 5, 0.55, 1.7),
    moistRaw: fbm(moisture, nx + 100, ny + 100, 4, 0.5, 1.3)
  };
}
...
export function buildSurfaceCells(seed, opts, profile) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      const { elevRaw, moistRaw } = sampleRawField(seed, lon, lat);
