...
import { each, rn, round, rw, meander, projectToNearestEdge } from "../utils/index.js";
import { seed, pack, grid, config, graphWidth, graphHeight } from "../src/state.js";
import { TIME, WARN } from "../utils/debug.js";
import { Lakes } from "./planet_lakes.js";
import { Features } from "./planet_features.js";
...
class RiverModule {
  ...
  MAX_FLUX_WIDTH = 1;
  LENGTH_FACTOR = 200;
  LENGTH_STEP_WIDTH = 1 / this.LENGTH_FACTOR;
  LENGTH_PROGRESSION = [1, 1, 2, 3, 5, 8, 13, 21, 34].map(n => n / this.LENGTH_FACTOR);
  lineGen = line().curve(curveBasis);
  ...
  addDownhill(initialCell) {
    ...
    while (cell) {
      ...
      const nextCell = cells.c[cell].sort((a, b) => heights[a] - heights[b])[0];
    ...
    if (river) {
      ...
      const type = this.getType({ i: riverId, length, parent });
      rivers.push({
        i: riverId,
        source,
        mouth,
  ...
  generate(allowErosion = true) {
    ...
    const addCellToRiver = (cellId, riverId) => {
    ...
    const drainWater = () => {
      ...
      const cellsNumberModifier = (config.cells / 10000) ** 0.25;
    ...
    const flowDown = (toCell, fromFlux, river) => {
    ...
    const defineRivers = () => {
      ...
      const defaultWidthFactor = rn(1 / (config.cells / 10000) ** 0.25, 2);
  ...
  resolveDepressions(h) {
    const { cells, features } = pack;
    const maxIterations = config.resolveDepressionsSteps;
    ...
    for (let iteration = 0; depressions && iteration < maxIterations; iteration++) {
      ...
      if (iteration < checkLakeMaxIteration) {
        for (const l of lakes) {
          ...
          const minHeight = min(l.shoreline.map(s => h[s]));
          if (minHeight >= 100 || l.height > minHeight) continue;

          if (iteration > elevateLakeMaxIteration) {
            l.shoreline.forEach(i => {
              h[i] = cells.h[i];
            });
            l.height = min(l.shoreline.map(s => h[s])) - 1;
            l.closed = true;
            continue;
          }
      ...
      for (const i of land) {
        const minHeight = min(cells.c[i].map(c => height(c)));
  ...
  addMeandering(riverCells, riverPoints = null) {
    ...
    const flux = new Array(points.length).fill(0);
  ...
  getOffset({ flux, pointIndex, widthFactor, startingWidth }) {
    ...
    const lengthWidth =
      ...
      (this.LENGTH_PROGRESSION[pointIndex] || this.LENGTH_PROGRESSION.at(-1));
  ...
  resolveLakeDrainFeature(lakeFeatureId) {
