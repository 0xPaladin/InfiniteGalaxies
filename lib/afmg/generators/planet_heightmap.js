...
import { PlateTectonicsGenerator } from "./plate-tectonics.js";
import { findGridCell } from "../utils/planet_graph.js";
import {
...
class HeightmapModule {
  ...
  /**
   * Generate a random [lon, lat] point across the full globe.
   */
  randomPointInBounds(rangeX, rangeY) {
    const lon = this.getPointInRange(rangeX, 360) - 180;
    const lat = this.getPointInRange(rangeY, 180) - 90;
    return [lon, lat];
  }
  ...
  addHill(count, height, rangeX, rangeY) {
    const addOneHill = () => {
      ...
      do {
        const [lon, lat] = this.randomPointInBounds(rangeX, rangeY);
        if (lon === undefined || lat === undefined) return;
        start = findGridCell(lon, lat, this.grid);
        limit++;
  ...
  addRange(count, height, rangeX, rangeY, startCellId, endCellId, randomness = 0.15) {
    ...
    const addOneRange = () => {
      ...
      const getRange = (cur, end) => {
        ...
        used[cur] = 1;
        let iterations = 0;
        const maxIterations = this.grid.cells.i.length;
        ...
        while (cur !== end && iterations < maxIterations) {
          ...
          this.grid.cells.c[cur].forEach(e => {
            if (used[e]) return;
            // Use spherical chord distance squared for proper globe topology
            let diff = chordDistanceSquared(p[end], p[e]);
      ...
      if (rangeX && rangeY) {
        // find start and end points using spherical coordinates
        const [startLon, startLat] = this.randomPointInBounds(rangeX, rangeY);
        ...
        do {
          endLon = Math.random() * 288 - 144; // -144 to 144
          endLat = Math.random() * 126 - 63;  // -63 to 63
          dist = Math.abs(endLat - startLat) + Math.abs(endLon - startLon);
          limit++;
        } while ((dist < 45 || dist > 120) && limit < 50);

        startCellId = findGridCell(startLon, startLat, this.grid);
        endCellId = findGridCell(endLon, endLat, this.grid);
      }
  ...
  addTrough(count, height, rangeX, rangeY, startCellId, endCellId, randomness = 0.2) {
    const addOneTrough = () => {
      ...
      if (rangeX && rangeY) {
        ...
        let endLat;
        do {
          [startLon, startLat] = this.randomPointInBounds(rangeX, rangeY);
          startCellId = findGridCell(startLon, startLat, this.grid);
          limit++;
        ...
        do {
          ...
          dist = Math.abs(endLat - startLat) + Math.abs(endLon - startLon);
          limit++;
        } while ((dist < 45 || dist > 180) && limit < 50);
  ...
  addStrait(width, direction = "vertical") {
    if (!this.heights || !this.grid) return;
    // Use a fraction of total cells for desired width
    const desiredWidth = Math.min(getNumberInRange(width), this.grid.cells.i.length / 300);
    if (desiredWidth < 1 && P(desiredWidth)) return;
    const used = new Uint8Array(this.heights.length);
    const vert = direction === "vertical";
    // Generate start and end points in spherical coordinates across the whole globe
    const startLon = vert ? Math.random() * 144 - 72 : -175;
    const startLat = vert ? -85 : Math.random() * 72 - 36;
    const endLon = vert
      ? -startLon - 36 + Math.random() * 72
      : 175;
    const endLat = vert
      ? 85
      : -startLat - 18 + Math.random() * 36;

    const start = findGridCell(startLon, startLat, this.grid);
    const end = findGridCell(endLon, endLat, this.grid);
  ...
  mask(power = 1) {
    ...
    this.heights = this.heights.map((h, i) => {
      ...
      // Whole globe mask: circular radial falloff from pole or equator
      const phi = (lat + 90) / 180; // 0 to 1
      let distance = Math.sin(phi * Math.PI); // 0 at poles, 1 at equator
  ...
  invert(count, axes) {
    ...
    // For spherical maps: reflect across equator (invert Y) or prime meridian/opposite (invert X)
    const oldHeights = Uint8Array.from(this.heights);
    const invertX = axes !== "y";
    const invertY = axes !== "x";

    this.heights = this.heights.map((_h, i) => {
      let [lon, lat] = this.grid.points[i];
      if (invertX) lon = lon > 0 ? lon - 180 : lon + 180;
      if (invertY) lat = -lat;
      const nearest = findGridCell(lon, lat, this.grid);
      return oldHeights[nearest];
    });
  }
  ...
  async generate(graph) {
    ...
    let id = config.template;
    Math.random = Alea(seed);

    // Normalize template name and resolve common aliases
    if (id) {
      if (id.toLowerCase() === "island") id = "highIsland";
      if (id.toLowerCase() === "plates" || id.toLowerCase() === "tectonic" || id.toLowerCase() === "tectonics") {
        id = "plates";
      } else {
        // Case-insensitive lookup in heightmapTemplates
        const matchedKey = Object.keys(heightmapTemplates).find(
          k => k.toLowerCase() === id.toLowerCase() || heightmapTemplates[k].name.toLowerCase() === id.toLowerCase()
        );
        if (matchedKey) id = matchedKey;
      }
    }

    let heights;
    if (id === "plates") {
      heights = PlateTectonicsGenerator.generate(graph, {
        nPlates: config.nPlates,
        mountainPower: config.mountainPower
      });
    } else {
      ...
      if (isTemplate) {
        ...
        try {
          ...
          console.warn(`Could not load precreated heightmap "${id}", falling back to "continents" template.`, err);
          heights = this.fromTemplate(graph, "continents");
        }
      }
    }
  ...
  fromPrecreated(graph, id) {
    return new Promise((resolve, reject) => {
      ...
      const n = graph.cells.i.length;
      // Use a square canvas approximation for image loading
      ...
      img.onerror = () => {
        canvas.remove();
        img.remove();
        reject(new Error(`Failed to load heightmap image ./heightmaps/${id}.png`));
      };
      img.onload = () => {
        if (!ctx) {
