  ...
  markupGrid() {
    ...
    for (let featureId = 1; queue[0] !== -1; featureId++) {
      ...
      while (queue.length) {
        ...
        const cellNeighbors = neighbors[cellId] || [];
        for (const neighborId of cellNeighbors) {
  ...
  markupPack() {
    ...
    const getCellsData = (featureType, firstCell) => {
      ...
      function findOnBorderCell(firstCell) {
        const isOnBorder = cellId => borderCells[cellId] || (neighbors[cellId] && neighbors[cellId].some(ofDifferentType));
        if (isOnBorder(firstCell)) return firstCell;

        const startCell = cells.i.filter(ofSameType).find(isOnBorder);
        if (startCell === undefined) return firstCell;

        return startCell;
      }

      function getFeatureVertices(startCell) {
        // Prefer a vertex shared with a cell of a different type (true border vertex)
        let startingVertex = cells.v[startCell].find(v => vertices.c[v].some(ofDifferentType));
        // Fall back to any vertex if no cross-type vertex exists (can happen
        // with degenerate cells from spherical Voronoi)
        if (startingVertex === undefined) startingVertex = cells.v[startCell][0];
    ...
    const packCellsNumber = i ? i.length : 0;
    ...
    console.log("markupPack complete, features:", pack.features.length);
