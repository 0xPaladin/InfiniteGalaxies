...
import { graphWidth, graphHeight } from "../src/state.js";

/**
 * Generates SVG path data for filling a shape defined by a chain of vertices.
 * @param vertices - The vertices object containing positions.
 * @param vertexChain - An array of vertex IDs defining the shape.
 * @returns SVG path data for the filled shape.
 */
const getFillPath = (vertices, vertexChain) => {
 ...
 * @param vertices - The vertices object containing positions.
 * @param vertexChain - An array of vertex IDs defining the border.
 * @param discontinue - A function that determines if the path should discontinue at a vertex.
 * @returns SVG path data for the border.
 */
const getBorderPath = (vertices, vertexChain, discontinue) => {
 ...
 * @param exit - The ID of the exit cell.
 * @param start - The ID of the starting cell.
 * @param from - An array mapping each cell ID to the cell ID it came from.
 * @returns An array of cell IDs representing the path from start to exit.
 */
const restorePath = (exit, start, from) => {
  const pathCells = [];
 ...
 * @param graph - The graph object containing cells and vertices.
 * @param getType - A function that returns the type of a cell given its ID.
 * @param options - Options to specify which isoline formats to generate.
 * @returns An object containing isolines for each type based on the specified options.
 */
export const getIsolines = (
  { cells, vertices, features },
  getType,
  options = { polygons: false, fill: false, halo: false, waterGap: false }
) => {
  const isolines = {};

  const checkedCells = new Uint8Array(cells.i.length);
  const addToChecked = cellId => {
  ...
  function addIsolineTo(type, vertices, vertexChain, isolines, options) {
 ...
 * @param cellsArray - An array of cell IDs defining the shape.
 * @param packedGraph - The packed graph object containing cells and vertices.
 * @returns SVG path data for the border of the shape.
 */
export const getVertexPath = (cellsArray, packedGraph = {}) => {
 ...
 * @param graph - The graph object containing cells and vertices.
 * @param getType - A function that returns the type of a cell given its ID.
 * @returns An object mapping each type to its pole of inaccessibility coordinates [x, y].
 */
export const getPolesOfInaccessibility = (graph, getType) => {
 ...
 * @param options - Options for connecting vertices.
 * @param options.vertices - The vertices object containing connections.
 * @param options.startingVertex - The ID of the starting vertex.
 * @param options.ofSameType - A function that checks if a cell is of the same type.
 * @param options.addToChecked - A function to mark cells as checked.
 * @param options.closeRing - Whether to close the path into a ring.
 * @returns An array of vertex IDs forming the connected path.
 */
export const connectVertices = ({ vertices, startingVertex, ofSameType, addToChecked, closeRing }) => {
 ...
 * @param start - The ID of the starting cell.
 * @param isExit - Returns true if `id` is the exit cell.
 * @param getCost - A function that returns the path cost from current cell to the next cell.
 * @param packedGraph - The packed graph object containing cells and their connections.
 * @returns An array of cell IDs of the path from start to exit, or null if not found.
 */
export const findPath = (start, isExit, getCost, packedGraph = {}) => {
...
export const parsePathPoints = path => {
  const points = [];
  let x = 0;
  let y = 0;

  const addPoint = (nextX, nextY, relative) => {
...
export const meander = (cells, cellPositions, options = {}) => {
  ...
  const anchorPoints = cells.map((cell, i) => {
    if (customAnchors?.[i]) return customAnchors[i];
    if (cell === -1) {
      const prevCell = cells[i - 1];
      const prev = prevCell !== undefined && prevCell >= 0 ? cellPositions[prevCell] : [0, 0];
...
function reflectAcrossLine(m, P, Q) {
  const dx = Q[0] - P[0];
  const dy = Q[1] - P[1];
...
function relaxAcuteAngles(points, anchorIndices) {
  ...
  for (let iter = 0; iter < RELAX_ITERATIONS; iter++) {
    const snapshot = points.map(p => [p[0], p[1]]);
    const at = k => snapshot[k];
    ...
    for (let i = 1; i < n - 1; i++) {
      ...
      const withFlip = k => (k === i ? flipped : snapshot[k]);
...
export function projectToNearestEdge(point, width, height) {
  const [x, y] = point;
...
// lineclip's default export is the line-clipper function; the polygon clipper
// is exposed as a property. Bind it so clipPoly can clip a single ring.
const clipPolygon = lineclip.polygon ? lineclip.polygon : lineclip;

export const clipPoly = (points, secure) => {
