...
import { Voronoi } from "../generators/voronoi.js";
 ...
 */
const placePoints = (graphWidth, graphHeight) => {
 ...
 */
export const shouldRegenerateGrid = (grid, expectedSeed, graphWidth, graphHeight) => {
 ...
 */
export const calculateVoronoi = (points, boundary) => {
 ...
 */
export const findGridCell = (x, y, grid) => {
  return (
 ...
 */
export const findGridAll = (x, y, radius, grid) => {
 ...
 */
export const findClosestCell = (x, y, radius = Infinity, pack) => {
 ...
 */
export const findAllInQuadtree = (x, y, radius, quadtree) => {
  let dx, dy, d2;

  const radiusSearchInit = (t, radius) => {
    t.result = [];
    t.x0 = t.x - radius;
    t.y0 = t.y - radius;
    t.x3 = t.x + radius;
 ...
 */
export const findAllCellsInRadius = (x, y, radius, packedGraph) => {
  const q = quadtree(packedGraph.cells.p.map(([px, py], i) => [px, py, i]));
 ...
 */
export const getGridPolygon = (i, grid) => {
 ...
 */
export const isLand = (i, packedGraph) => {
 ...
 */
export const isWater = (i, packedGraph) => {
  return packedGraph.cells.h[i] < 20;
};
...
export function reGraph(grid) {
  ...
  // The "exclude all deep ocean points" rule above assumes there's always
  // SOME coastline elsewhere in the map to pack in fine detail — true for
  // AFMG's usual continent-scale maps, but a per-cell region generated from
  // an open-ocean surface cell can legitimately be 100% deep water with no
  // coastline anywhere in it, so EVERY cell gets filtered out and newCells
  // stays empty. That leaves pack.cells.i.length at 0, which makes
  // markupPack (features.js) bail out early without ever setting
  // pack.features, crashing every downstream .forEach on it (Lakes/Rivers).
  // Fall back to packing the raw grid unfiltered so there's still a real,
  // all-water cell set to both pack correctly and actually render.
  if (!newCells.p.length) {
    for (const i of gridCells.i) {
      const [x, y] = points[i];
      addNewPoint(i, x, y, gridCells.h[i]);
    }
  }
