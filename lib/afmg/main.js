...
export async function generateMap(options = {}) {
  ...
  // A caller-supplied heightmap function bypasses the template-driven generator
  // entirely — everything downstream (Features/Lakes/climate/Rivers/Biomes/Ice)
  // only ever reads grid.cells.h, so this is a safe seam. Contract: return (or
  // resolve to) a Uint8Array-like of length grid.points.length, values 0..100,
  // h >= 20 meaning land (see generators/features.js).
  grid.cells.h = typeof options.heightmap === "function"
    ? await options.heightmap(grid, config)
    : await path.Heightmap.generate(grid);
  setPack({});
