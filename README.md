...
> That ~5% pond figure predates a later fix: ponds used to additionally be capped at 400 connected
> tiles (~1600km²), rendering anything bigger as plain dry ground on the (once-true) assumption that
> only a window-boundary artifact could get that large. Once terrain gained real 2km relief, that
> stopped holding — legitimate, fully-enclosed basins with a real detected rim commonly exceed 400
> tiles now, and the size cap was cutting them off mid-shoreline, a hard color cliff where a real lake
> met plain ground. The cap is gone; a basin is now excluded only if its connected component touches
> the sampling grid's own outer edge (proof flow never found a lower way out inside the grid — the
> actual artifact case). Water coverage is therefore no longer capped near 5% — a region whose terrain
> genuinely sits in one broad low basin can read as substantially wetter, which is the physically
> honest result rather than an accident of tile-counting.
